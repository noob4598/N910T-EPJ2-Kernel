...
#define LOG_TAG "JackAlsaDriver"

using namespace android;

#define CHECK_SEQ_TIME(type,seq) \
    (mJackLogger) ? mJackLogger->checkSystemTime(type,seq) : jack_error("mJackLogger is null");
#endif
                         ...
                         const char* midi_driver_name)
{
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
    if(mJackLogger != NULL){
        mJackLogger->setDriverDevices(playback_driver_name, capture_driver_name);
        CHECK_SEQ_TIME(JackLogger::LOG_TIME_AD_CONNECT, JackLogger::SAVE_LOG_START)
    }
#endif
    ...
    fDriver = alsa_driver_new ((char*)"alsa_pcm", (char*)playback_driver_name, (char*)capture_driver_name,
                               ...
                               midi);
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
    CHECK_SEQ_TIME(JackLogger::LOG_TIME_AD_CONNECT, JackLogger::SAVE_LOG_END)
#endif
...
int JackAlsaDriver::Start()
{
    ...
    if (res >= 0) {
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
        CHECK_SEQ_TIME(JackLogger::LOG_TIME_AD_START, JackLogger::SAVE_LOG_START)
#endif
            
        res = alsa_driver_start((alsa_driver_t *)fDriver);
        
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
        CHECK_SEQ_TIME(JackLogger::LOG_TIME_AD_START, JackLogger::SAVE_LOG_END)
#endif
...
void JackAlsaDriver::ReadInputAux(jack_nframes_t orig_nframes, snd_pcm_sframes_t contiguous, snd_pcm_sframes_t nread)
{
    ...
    if(mJackLogger){
        if(mJackLogger->isStartedPcmDump()) {
            jack_default_audio_sample_t* buf = (jack_default_audio_sample_t*)fGraphManager->GetBuffer(fCapturePortList[0], orig_nframes);
            mJackLogger->dumpReadData((float*)((alsa_driver_t *)fDriver)->capture_addr[0],contiguous);
        }
    }
#endif    
}
