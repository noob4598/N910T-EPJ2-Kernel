...
#define LOG_TAG "JackAudioDriver"

#define CHECK_PROCESS_TIME(type,seq) \
    if(mJackLogger ) \
        if(mJackLogger->isFirstProcess()) mJackLogger->checkSystemTime(type, seq);
#endif
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
    if(mJackLogger )
        if(mJackLogger->isFirstProcess()) mJackLogger->checkSystemTime(JackLogger::LOG_TIME_AD_FIRSTPROCESS, JackLogger::SAVE_LOG_START);
#endif
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
    if(mJackLogger )
        if(mJackLogger->isFirstProcess()) mJackLogger->checkSystemTime(JackLogger::LOG_TIME_AD_FIRSTPROCESS, JackLogger::SAVE_LOG_END);
#endif
...
#ifdef __ANDROID__
    if(fEngineControl->fSyncModePending == true) {
        fEngineControl->UpdateSyncMode(true);
        jack_log("JackAudioDriver::ProcessGraphAsyncMaster: mode change async to sync");
        for (int i = 0; i < fEngineControl->fDriverNum; i++)
...
#ifdef __ANDROID__
    if(fEngineControl->fSyncModePending == false) {
        fEngineControl->UpdateSyncMode(false);
        jack_log("JackAudioDriver::ProcessGraphSyncMaster: mode change sync to async");
        for (int i = 0; i < fEngineControl->fDriverNum; i++)
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
void JackAudioDriver::CreateJackLog()
{
    typedef void * (*fJackLogger)();
    void* tLoggerLib = NULL;
    ALOGE("CreateJackLog");
    dlerror();
    tLoggerLib = dlopen("libjacklogger.so", RTLD_NOW);
    if(tLoggerLib == NULL) {
        jack_error("CreateJackLog dlopen failed : about control module\n");
        fprintf(stderr, "%s\n", dlerror());
    }else{
        fJackLogger iJackLoggerInstance;
        iJackLoggerInstance = (fJackLogger) dlsym(tLoggerLib, "Instance");
         if(iJackLoggerInstance != NULL){
            mJackLogger = (JackLogger *) iJackLoggerInstance();
        }else{
            jack_error("CreateJackLog getInstance is NULL, error '%s'", dlerror());    
        }       
    }

    char* error = NULL;
    if((error = (char*)dlerror()) != NULL) {
        jack_error("CreateJackLog fail to load creator symbol \n");
        fprintf(stderr, "%s\n", error);
    }
...
void JackAudioDriver::RemoveJackLog()
{
    ALOGE("RemoveJackLog");
    if(mJackLogger != NULL){
        jack_log("JackAudioDriver delete instance %p", mJackLogger);
        if(mJackLogger->deleteInstance() == NULL){
            jack_error("JackAudioDriver JackLogger is destructed");
            mJackLogger = NULL;
        }
    }
}
